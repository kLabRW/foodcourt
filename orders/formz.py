...
from restaurant_detail.form import OptionalForm
#from optionalitems.models import OptionalItems
...
class PartialOrderItemform(forms.Form):
		...
		def __init__(self,item ,*args, **kwargs):
			...
			self.fields['option'].queryset = Optional_Item.objects.filter(item=item)
		
#----------------------------------------------------------------------------------

#class My_Order(forms.Form):
#	option = forms.ModelMultipleChoiceField(queryset = OptionalItems.objects.all(), widget = RadioSelect())
#	quantity = forms.IntegerField(widget = TextInput())

#---------------------------------------------------------------
#class PartialOrderItemForm(OptionalForm):
#	optional_item = forms.ModelMultipleChoiceField(queryset = Item.objects.all(), widget = RadioSelect())
#	class Meta:
#		model = OrderItem
#		fields = ('quantity')
#		widgets = {
